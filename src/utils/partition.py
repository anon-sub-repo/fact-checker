...
class Partition:
    ...
    def can_merge(self, partition):
        return (not torch.logical_and(self.classes, partition.classes).any()) and not torch.logical_and(self.partition != -1, partition.partition != -1).any()
    
    def merge_partitions(self, partition):
        self.partition = torch.where(self.partition == -1, partition.partition, self.partition)
...
def partition_predictions(sep_index, starts, ends, target_spans, threshold=0, top_k=3):
    targ = target_spans[0]
    a = [[int(z) for z in x.argsort(descending=True) if x[z] > threshold][:top_k] for x in starts[:, :sep_index]]
    b = [[int(z) for z in x.argsort(descending=True) if x[z] > threshold][:top_k] for x in ends[:, :sep_index]]
    ...
    valid_combos = [[z for z in x if z[0] <= z[1] and z[0] != 0 and z[1] != 0 and ((z[0] < targ[0] and z[1] < targ[0]) or (z[0] > targ[1] and z[1] > targ[1]))] + [(0,0)] for x in all_combinations]

    # sort the combinations by score
    sorted_combos = [sorted(x, key=lambda y: starts[i, y[0]] + ends[i, y[1]], reverse=True) for i, x in enumerate(valid_combos)]
    ...
    for p in possible_partitions:
        ...
        for i, span in enumerate(p):
            if partition.check_span(span[0], span[1]):
                partition.insert_span(span[0], span[1], i, starts[i, span[0]] + ends[i, span[1]])
            else:
                break
        else:
