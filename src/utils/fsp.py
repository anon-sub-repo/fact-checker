...
class FrameSemanticParsingOutput:
    def __init__(self, 
                 ...
                 start_fe_logits: Optional[torch.FloatTensor] = None, 
                 end_fe_logits: Optional[torch.FloatTensor] = None, 
    ...
    def __repr__(self):
        return (f"FrameSemanticParsingOutput:\n\tLoss: {self.loss}\n\tFrame Logits: {self.frame_logits}\n"
                f"\tStart Logits: {self.start_fe_logits}\n\tEnd Logits: {self.end_fe_logits}\n"
...
class VoteFSPDataset(Dataset):
    def __init__(self, dataset: List[dict], device: torch.device, tokenizer):
    ...
    def __getitem__(self, idx: int):
        input_ids = pad_sequence([x["input_ids"] for x in self.dataset[idx]], 
                                 batch_first=True, 
                                 padding_value=self.tokenizer.pad_token_id)
        
        attention_mask = pad_sequence([x["attention_mask"] for x in self.dataset[idx]], 
                                      batch_first=True)
...
def build_model_inputs(train_vote_samples: List[dict], tokenizer, vote_frame_info: dict, label: int = 1):
    train_raw_dataset = []
    ...
    for sample in train_vote_samples:
        ...
        target_span = (sample_toks.char_to_token(sample["target_span"][0]), 
                       sample_toks.char_to_token(sample["target_span"][1] - 1))
        
        sample_fes = {fe_name: (0, 0) for fe_name in vote_frame_info["fes"]}
        
        for sample_fe in sample["fe_spans"]:
            fe_name = sample_fe[-1]
            fe_span = (sample_toks.char_to_token(sample_fe[0]), 
                       sample_toks.char_to_token(sample_fe[1]))
            sample_fes[fe_name] = fe_span
        
        for fe_name, span in sample_fes.items():
            fe_toks = tokenizer.encode_plus(
                f"{sample['sentence']}{tokenizer.sep_token}Vote{tokenizer.sep_token}{fe_name}", 
                return_tensors="pt"
            )
            sample_inputs.append({
                "input_ids": fe_toks["input_ids"].squeeze(), 
                "attention_mask": fe_toks["attention_mask"].squeeze(), 
